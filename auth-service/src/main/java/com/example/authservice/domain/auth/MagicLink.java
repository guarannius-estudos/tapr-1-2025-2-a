...
public class MagicLink {
    ...
    public MagicLink(UUID userId, HashedToken hashedToken, ExpiresAt expiresAt) {
        if (userId == null) {
            throw new IllegalArgumentException("userId é obrigatorio.");
        }
    ...
    public void consume(Instant now) {
        if (this.isConsumed()) {
            throw new IllegalStateException("Link já consumido.");
        }
        if (this.isExpired(now)) {
            throw new IllegalStateException("Link já expirado.");
        }
